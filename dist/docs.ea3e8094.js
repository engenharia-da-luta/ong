...
document.addEventListener("click", (event)=>{
    dropdowns.forEach((dropdown)=>{
        const trigger = dropdown.querySelector(".dropdown__trigger");
        if (dropdown.classList.contains("dropdown--active") && trigger && !trigger.contains(event.target)) dropdown.classList.remove("dropdown--active");
    });
});
...
const cashPositiveTotalElement = document.querySelector(".cash--positive-total");
const cashNegativeTotalElement = document.querySelector(".cash--negative--total");
...
cashPositiveElements.forEach((element)=>{
    ...
    if (match && match[1]) {
        ...
        if (!isNaN(valorNum)) cashSumPositive += valorNum;
    }
});
cashNegativeElements.forEach((element)=>{
    ...
    if (match && match[1]) {
        const valorStr = match[1];
        const valorNum = parseFloat(valorStr.replace(",", "."));
        if (!isNaN(valorNum)) cashSumNegative += valorNum;
    }
});
const cashSumPositiveFormatado = `R$${cashSumPositive.toFixed(2).replace(".", ",")}`;
const cashSumNegativeFormatado = `R$${cashSumNegative.toFixed(2).replace(".", ",")}`;
...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    function filterCards(category) {
        cards.forEach((card)=>{
            if (category === "all") card.classList.add("story__card--selected");
            else {
                const isSelected = card.classList.contains(`story__card--${category}`);
                if (isSelected) card.classList.add("story__card--selected");
                else card.classList.remove("story__card--selected");
            }
        });
    }
...
function calculateStock() {
    ...
    if (totalStock) {
        const result = Array.from(cards).reduce((acumulador, card)=>{
            ...
            const elementStock = card.querySelector(".give-card__stock .calc--multiply");
            if (elementPrice && elementStock) {
                const priceText = elementPrice.textContent;
                const stockText = elementStock.textContent;
                const price = parseFloat(priceText);
                const stock = parseInt(stockText);
                if (!isNaN(price) && !isNaN(stock)) return acumulador + price * stock;
            }
            return acumulador;
        }, 0);
...
//# sourceMappingURL=docs.ea3e8094.js.map
